...
- **Host automation socket:** `VPhoneHostControl` listens on `vphone.sock` next to `config.plist` (default `vm/vphone.sock`). JSON line in/out; forwards guest-agent ops (`app_*`, `clipboard_*`, `file_*`, `ipa_install`) via `VPhoneControl` / vsock. See README **Automation**.
- **Make boot passthrough:** `Makefile` `boot` / `boot_less` / `boot_dfu` append `$(EXTRA_ARGS)` to the bundled CLI invocation.
- **SIGPIPE:** Host `main.swift` and guest `vphoned.m` call `signal(SIGPIPE, SIG_IGN)` so closed pipe/socket peers do not terminate the process with exit `141`.
- **Serial log files:** `VPhoneVirtualMachine` mirrors PL011 guest serial to `vm/logs/serial-*.log` and `vm/logs/serial.log` in addition to stdout.
- **Host automation socket:** `VPhoneHostControl` listens on `vphone.sock` next to `config.plist` (default `vm/vphone.sock`). JSON line in/out; forwards guest-agent ops (`app_*`, `clipboard_*`, `file_*`, `ipa_install`) via `VPhoneControl` / vsock. See README **Automation**.
- **Make boot passthrough:** `Makefile` `boot` / `boot_less` / `boot_dfu` append `$(EXTRA_ARGS)` to the bundled CLI invocation.
- **SIGPIPE:** Host `main.swift` and guest `vphoned.m` call `signal(SIGPIPE, SIG_IGN)` so closed pipe/socket peers do not terminate the process with exit `141`.
- **Serial log files:** `VPhoneVirtualMachine` mirrors PL011 guest serial to `vm/logs/serial-*.log` and `vm/logs/serial.log` in addition to stdout.
