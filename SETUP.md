...
Download `iPhone17,3_26.1_23B85_Restore.ipsw` from [ipsw.me](https://ipsw.me) and place it in `firmwares/`:

```bash
mkdir -p firmwares
# download manually → firmwares/iPhone17,3_26.1_23B85_Restore.ipsw
```
...
# copy the downloaded IPSW to the repo's firmwares/ directory:
cp /path/to/downloaded/pcc_os_23B85.ipsw /path/to/repo/firmwares/pcc_os_23B85.ipsw
```

If you used Option B and already have both IPSWs in `firmwares/`, run the mix:

```bash
bash setup_download_fw.sh  # safe to re-run — skips already-downloaded files
```

Result: `firmwares/firmware_patched/iPhone17,3_26.1_23B85_Restore/` with mixed components from both IPSWs and the custom `BuildManifest.plist` / `Restore.plist` from `contents/`.
...
- Firmware is mixed into `firmwares/firmware_patched/iPhone17,3_26.1_23B85_Restore/`
...
| Path | Why removable |
|---|---|
| `build_tart.sh` | Superseded by `setup_bin.sh` |
| `tart-vphone.entitlements` | Duplicate — lives in `oems/super-tart/Resources/` |
| `setup_vm.sh` | One-time VM bootstrap helper, VM already exists |
| `SESSION_PROGRESS.md` | Session scratchpad |
| `nvram.bin` (repo root) | Stray file, actual nvram is in `.tart/vms/vphone/` |
| `macos-ipsw/` | Only needed for `tart create` scaffold, VM already created |
| `Ramdisk/` | Regenerated by `prepare_ramdisk.py` every run |
| `patch_oems/` | Empty placeholder |
| `checksums.txt.sha1` | Only relevant for distributing bin/ |
| `shsh/` | SHSH is regenerated; ramdisk img4s are regenerated |
| `docs/README_old.md` | Superseded |
| `docs/GAPS.md`, `docs/full_log.md` | Working notes |
| `docs/isSupported_deep_analysis.md`, `docs/vrevm_analysis_macos26b3.md` | Analysis scratch |
| `patch_scripts/__pycache__/` | Python bytecode cache |
| `patch_scripts/avpbooter_swap_backup/` | Backup artifacts |
| `patch_scripts/avpbooter_system_artifacts/`, `avpbooter_system_artifacts_test/` | Intermediate artifacts |
| `patch_scripts/restore_*.log` | Restore session logs |

### Cannot remove yet — `original-research/` is still needed

`install_jb_basebin.py` and `install_jb_procursus.py` fall back to `original-research/super-tart-vphone/CFW/jb/` for files not yet copied into the local `jb/` directory:

| File needed | Currently only in |
|---|---|
| `BaseBin/systemhook`, `BaseBin/launchdhook` | `original-research/CFW/jb/BaseBin/` |
| `bootstrap-iphoneos-arm64.tar.zst` | `original-research/CFW/jb/` |
| `org.coolstar.sileo_2.5.1_iphoneos-arm64.deb` | `original-research/CFW/jb/` |
| `custom_26.1/libellekit.dylib` | `original-research/CFW/custom_26.1/` |
| `tools/optool` | `original-research/CFW/tools/` |

To fully remove `original-research/`, copy these into the local tree first:

```bash
REPO="$(pwd)"
CFW="$REPO/original-research/super-tart-vphone/CFW"
cp -r "$CFW/jb/BaseBin"                              "$REPO/jb/"
cp    "$CFW/jb/bootstrap-iphoneos-arm64.tar.zst"    "$REPO/jb/"
cp    "$CFW/jb/org.coolstar.sileo_2.5.1_iphoneos-arm64.deb" "$REPO/jb/"
cp -r "$CFW/custom_26.1"                             "$REPO/"
mkdir -p "$REPO/tools"
cp    "$CFW/tools/optool"                            "$REPO/tools/"
```

Then `original-research/` can be removed safely.

**Always keep:** `patch_scripts/signcert.p12`, `jb/`, `patch_scripts/metal_cache/`, `patch_scripts/metal_plugin/`, `contents/`, `vphone-cli/`.
...
After restore, follow `VPHONE_RUNBOOK.md` Sections 2–6 (ramdisk → rootfs → normal boot).
